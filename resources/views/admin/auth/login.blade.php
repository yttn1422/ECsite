<x-guest-layout>
    ...
    <form method="POST" action="{{ route('admin.login') }}">
        ...
        <h1 class="sm:text-xl text-xl font-medium title-font mb-2 text-gray-700">出品者</h1>
        </div>
        <!-- Email Address -->
        <div>
        ...
        <div class="text-center flex space-x-4">
            <div class="w-1/2 block mt-4">
                <a href="{{ route('login') }}" class="inline-flex items-center underline">
                    {{ __('一般ユーザーはこちら') }}
                </a>
            </div>

            <div class="w-1/2 block mt-4">
                <a href="{{ route('admin.register') }}" class="inline-flex items-center underline">
                    {{ __('新規登録はこちら') }}
                </a>
            </div>
        </div>
